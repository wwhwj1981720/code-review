...
public class FrameTaskRead {
	...
	public FrameTaskRead(String mdir) {
		super();
	...
	public FrameTaskRead(String mdir,CFrame cf) {
		super();
		Dir dir=new Dir(mdir);
		Const.resut=dir.getPath()+Const.resutpr;
		Const.funDir=dir.getPath()+Const.funDirpr;
		...
		{ 
			if(filetypelist!=null) 
			{
				...
				for (String key:filetypelist.keySet()) 
				{
					...
					java.util.concurrent.Callable doTask = new SaveMidFileDoTask(lf,key,totalresult,cFrame);
	...
	public static void main(String args[]) throws IOException, NoSuchAlgorithmException
	{
		...
		FrameTaskRead rd = new FrameTaskRead(mdr);
		rd.scanDirectry(mdr);
		try {
			rd.travelMap(suffix);
