...
public class FrameTaskFun {
	...
	public FrameTaskFun(String mdir) {
		super();
	...
	public FrameTaskFun(String mdir,CFrame cf) {
		super();
		Dir dir=new Dir(mdir);
		Const.resut=dir.getPath()+Const.resutpr;
		Const.funDir=dir.getPath()+Const.funDirpr;
		...
		{ 
			if(filetypelist!=null) 
			{
				...
				for (String key:filetypelist.keySet()) 
				{
					...
					java.util.concurrent.Callable doTask = new DoTaskFrame(lf,key,totalresult,cFrame);
	...
	public static void main(String args[]) throws IOException, NoSuchAlgorithmException
	{
		...
		FrameTaskFun rd = new FrameTaskFun(mdr);
		rd.scanDirectry(mdr);
		try {
			rd.travelMap(suffix);
