 ...
 *
 */
public class DoTaskFrame extends DoTask implements Callable
{
	...
	//
 public DoTaskFrame(List<String> lf,String suffix,CFile totalresult,CFrame frame)
 {
	 super(lf,suffix,totalresult);
	 //fj=new JsFunctionJudge();//
	 Dir filedir=new Dir(Const.funDir);
	 //Dir worddir=new Dir(Const.wordDir);
	 funresult=new CFile(Const.funDir+type+"fun.csv");//每种类型汇总文件分别输出结果
 ...
 public DoTaskFrame(List<String> lf,String suffix,CFile totalresult)
 {
	 super(lf,suffix,totalresult);
	 //fj=new JsFunctionJudge();//
	 Dir filedir=new Dir(Const.funDir);
